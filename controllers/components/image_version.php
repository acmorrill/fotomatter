...
class ImageVersionComponent extends Object {
	 ...
	 */
	function version($options = array('image'=>null, 'size'=>array(75, 75), 'quality'=>85, 'crop'=>false, 'letterbox'=>null, 'force_letterbox_color'=>false, 'sharpen'=>true, 'absolute_path' => false)) {
		...
		// get full path of source file	(note: a beginning slash doesn't matter, the File class handles that I believe)
                if ($options['absolute_path'] == false) {
                     $originalFile = new File($this->webRoot . $source);
                } else {
                     $originalFile = new File($source);
                }
		...
		if(file_exists($originalFile->Folder->path.DS.$thumb_size_x.'x'.$thumb_size_y.DS.$originalFile->name().'.'.$originalFile->ext())):
			...
			if( date('YmdHis', $existingFile->lastChange()) > date('YmdHis', $originalFile->lastChange()) ):
				...
				return $this->formatPath($existingFile->Folder->path.DS.$existingFile->name().'.'.$existingFile->ext());				
			endif;
		endif;
		...
		$finalPath = $outputPath->Folder->path.DS.$outputPath->name().'.'.$outputPath->ext();
