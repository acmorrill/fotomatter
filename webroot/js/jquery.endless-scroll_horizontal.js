...
(function($){
  ...
  $.fn.endlessScroll_horizontal = function(options) {
    ...
    setInterval(function() {
      if (didScroll && firing === true) {
        ...
        if (scrollTarget == document || scrollTarget == window) {
          is_scrollable = $(document).width() - $(window).width() <= $(window).scrollLeft() + options.bottomPixels;
        } else {
          // calculates the actual width of the scrolling container
//          if (inner_wrap.length == 0) {
//            inner_wrap = $(scrollTarget).wrapInner("<div class=\"endless_scroll_inner_wrap\" />").find(".endless_scroll_inner_wrap");
//          }
//          is_scrollable = inner_wrap.length > 0 && (inner_wrap.width() - $(scrollTarget).width() <= $(scrollTarget).scrollLeft() + options.bottomPixels);
          is_scrollable = jQuery(scrollTarget).scrollLeft() + jQuery(scrollTarget).width() > jQuery(scrollTarget)[0].scrollWidth - options.bottomPixels;
//		  console.log (is_scrollable);
        }
