...
class ThemeLogoHelper extends AbstractThemeLogoHelper {
	...
	protected function _create_theme_base_logo($base_logo_file_path) {
		...
		$im = @imagecreatetruecolor($string_pixel_width + (2*$padding), $font*2 + (2*$padding));
		imagesavealpha($im, true);
		imagealphablending($im, false);
		$clear = imagecolorallocatealpha($im, 255, 255, 255, 127);
		$black = imagecolorallocatealpha($im, 51, 51, 51, 0);
		$red = imagecolorallocatealpha($im, 246, 63, 63, 0);
		imagefill($im, 0, 0, $clear);
		imagettftext($im, $font, 0, $padding, $padding+$font, $black, $font_file, $firstname);
		imagettftext($im, $font, 0, $first_name_pixel_width+$padding, $font+$padding, $black, $font_file, $lastname);
		imagettftext($im, $font, 0, $first_name_pixel_width+$padding, ($font*2)+$padding, $black, $font_file, $company_name);
